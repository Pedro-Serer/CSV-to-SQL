<?php
    ...
    */
    ...
    if (file_exists($diretorio)) {
        ...
        if(isset($argv[2])){
            if ($argv[2] == "-v") {
                saida_cria_tabela($cabCSV, $tabela, $tipo, $tamanho, $max);
                saida_insere_dados($linhasCSV, $tabela, $cabCSV, $max);
            } else {
                die("\nParametro com valor \"{$argv[2]}\" desconhecido\n");
            }
        }
        
        list($sucesso1, $colunas) = cria_tabela(SQL, $cabCSV, $tabela, $tipo, $tamanho, $max);
        $sucesso2 = insere_dados(SQL, $linhasCSV, $tabela, $colunas, $tipo);
